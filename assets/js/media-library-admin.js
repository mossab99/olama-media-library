jQuery(function ($) {
    ...
    function showDirectStartFailure(uploadState, data, fallbackMessage) {
        const error = typeof data === 'object' && data !== null ? data : {};
        const message = error.message_ar || error.message || fallbackMessage || cfg.i18n.direct_browser_failed;
        showDirectFallback(uploadState, message, error.error_code || 'direct_session_create_failed', {
            xhr_status: '',
            drive_http_status: error.drive_http_status || '',
            chunk_index: Number.isInteger(error.failed_chunk_index) ? error.failed_chunk_index : 0,
            loaded_bytes: 0,
            total_bytes: uploadState.file.size,
            stage: error.stage || 'direct_session_create',
            message_en: error.message_en || message
        }, 'direct_upload_failed', error.retryable !== false);
    }
    ...
    function uploadFileDirect(uploadState) {
        ...
        refreshUploadNonce().done(function (nonceResponse) {
            if (!nonceResponse.success || !nonceResponse.data || !nonceResponse.data.drive_authenticated) {
                showDirectStartFailure(uploadState, nonceResponse.data, cfg.i18n.session_or_permission_expired);
                return;
            }
            ...
            $.post(cfg.ajaxurl, {
                ...
                if (!response.success || !response.data || !response.data.upload_url) {
                    showDirectStartFailure(uploadState, response.data, cfg.i18n.direct_browser_failed);
                    return;
                }
                ...
                showDirectStartFailure(uploadState, responseData, cfg.i18n.direct_browser_failed);
            });
        }).fail(function () {
            showDirectStartFailure(uploadState, null, cfg.i18n.session_or_permission_expired);
        });
    }
    ...
    function showDirectFallback(uploadState, message, errorCode = 'direct_browser_error', diagnostics = {}, eventType = 'direct_upload_browser_error', allowActions = true) {
        const $progress = uploadState.progress;
        setUploadState(uploadState.upload_id, {
            status: 'failed',
        ...
        uploadState.statusElement.text(allowActions ? `${fullMessage} ${cfg.i18n.direct_fallback_available}` : fullMessage);
    ...
    function auditFilters() {
        return {
            academic_year_id: $('#audit-year-id').val(),
            semester_id: $('#audit-semester').val(),
            grade_id: $('#audit-grade').val(),
            subject_id: $('#audit-subject').val()
        };
    }

    function setWorkflowStep(step, status) {
        const $card = $(`.olama-workflow-card[data-workflow-step="${step}"]`);
        $card.removeClass('is-active is-complete is-ready');
        if (status) $card.addClass(`is-${status}`);
    }
    ...
    $('#audit-year-id').on('change', function () {
        ...
        $.get(cfg.ajaxurl, { action: 'olama_media_get_semesters', nonce: cfg.nonce, academic_year_id: $(this).val() })
            .done(function (response) {
                let html = '';
                if (response.success) response.data.forEach((semester) => { html += `<option value="${esc(semester.id)}">${esc(semester.semester_name)}</option>`; });
                $semester.html(html).prop('disabled', false);
    ...
    $('#audit-grade').on('change', function () {
        ...
        $.get(cfg.ajaxurl, { action: 'olama_get_subjects', nonce: cfg.nonce, grade_id: gradeId }).done(function (response) {
            let html = `<option value="">${esc(cfg.i18n.select)}</option>`;
            if (response.success) response.data.forEach((subject) => { html += `<option value="${esc(subject.id)}">${esc(subject.subject_name)}</option>`; });
            $subject.html(html).prop('disabled', false);
            updateAuditScopeState();
        });
    });
    ...
    $('#btn-audit-scope').on('click', function () {
        if (!auditFilters().subject_id) return;
        resetLinkCheckResults();
        updateAuditScopeState();
        setWorkflowStep(2, 'active');
        $('#btn-drive-mapping-candidates').prop('disabled', false).trigger('click');
    });
    ...
    $(document).on('click', '.btn-manual-drive-mapping', function () {
        ...
        const scope = [$('#audit-year-id option:selected').text(), $('#audit-semester option:selected').text(), $('#audit-grade option:selected').text(), $('#audit-subject option:selected').text()].join(' / ');
    ...
    function renderReconciliationSummary(data) {
        ...
        $('#reconciliation-summary').find('p').html(`<strong class="olama-summary-title">${esc(cfg.i18n.reconciliation_complete)}</strong><span class="olama-reconciliation-metrics">${metrics.map(function (metric) {
            return `<span><small>${esc(metric[0])}</small><strong>${esc(Number(metric[1] || 0))}</strong></span>`;
        }).join('')}</span>`);
    }
    ...
    $(document).on('click', '.btn-reconciliation-assign, .btn-reconciliation-reject', function () {
        ...
        $.post(cfg.ajaxurl, {
            ...
            if (response.data.decision_status === 'rejected') {
                $row.find('.reconciliation-lesson').val('');
            }
            const previousStatus = $row.attr('data-decision-status') || 'pending';
            $row.attr('data-decision-status', response.data.decision_status);
            const report = $('#reconciliation-summary').data('report') || {};
            report.decisions = report.decisions || { pending: 0, approved: 0, manual: 0, rejected: 0 };
            if (previousStatus !== response.data.decision_status) {
                if (Object.prototype.hasOwnProperty.call(report.decisions, previousStatus)) report.decisions[previousStatus] = Math.max(0, Number(report.decisions[previousStatus] || 0) - 1);
                if (Object.prototype.hasOwnProperty.call(report.decisions, response.data.decision_status)) report.decisions[response.data.decision_status] = Number(report.decisions[response.data.decision_status] || 0) + 1;
            }
    ...
    $('#btn-reconciliation-readiness').on('click', function () {
        ...
        $.post(cfg.ajaxurl, {
            ...
            const data = response.data;
            const decisions = data.decisions || {};
            const message = `الإجمالي: ${data.total} · المقبول: ${data.accepted} · المعتمد: ${decisions.approved || 0} · اليدوي: ${decisions.manual || 0} · المرفوض: ${decisions.rejected || 0} · المعلّق: ${decisions.pending || 0} · التعارضات: ${(data.conflicts || []).length}`;
            ...
            const plannedMessage = `ترقية روابط مطابقة: ${planned.promote_same_target || 0} · إعادة توجيه روابط آلية معلّقة: ${planned.reassign_target || 0}`;
