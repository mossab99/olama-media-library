...
assert_folder_plan_safety(strpos($source, "parent_drive_folder_id === (string) \$mapping->drive_folder_id") !== false, 'Candidates must be direct children of the confirmed subject Drive ID.');
...
assert_folder_plan_safety(strpos($source, 'invalid_curriculum_unit_name') !== false, 'Invalid curriculum unit names must block creation.');
...
foreach (array('olama_drive_folder_plans', 'olama_drive_folder_plan_items', 'subject_mapping_id', 'subject_drive_folder_id', 'plan_hash', 'planned_action', 'parent_drive_folder_id', 'candidate_drive_folder_ids') as $schema) {
    assert_folder_plan_safety(strpos($db, $schema) !== false, "Folder plan schema must include {$schema}.");
}
assert_folder_plan_safety(strpos($plugin, "OLAMA_MEDIA_LIBRARY_VERSION', '2.7.0'") !== false, 'Plugin version must be 2.7.0.');
assert_folder_plan_safety(strpos($plugin, "OLAMA_MEDIA_LIBRARY_DB_VERSION', '2.7.0'") !== false, 'Database version must trigger folder plan table creation.');

require_once $root . '/includes/class-olama-media-folder-provisioning.php';
$service = new Olama_Media_Folder_Provisioning(new stdClass(), new stdClass(), new stdClass());
